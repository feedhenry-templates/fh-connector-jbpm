...
app.use('/jbpm', proxy(jbpmHost + ':' + jbpmPort, {
  decorateRequest: function(proxyReq) {
    if (jbpmUsername && jbpmPassword) {
      var b64 = new Buffer(jbpmUsername + ':' + jbpmPassword).toString('base64');
      proxyReq.headers['Authorization'] = 'Basic '+ b64;
    }
    return proxyReq;
  }
}));
...
app.listen(port, host, function() {
  var now = new Date();
  console.log('App started at: ' + now +' on port: ' + port); // eslint-disable-line
});
